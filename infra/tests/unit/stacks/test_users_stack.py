"""CDK assertion tests for UsersStack."""
...
def _make_template(stack_id: str = "TestStack") -> Template:
    """Synthesise UsersStack and return its CloudFormation template."""
...
def test_lambda_create_user_handler(template: Template) -> None:
    template.has_resource_properties(
        "AWS::Lambda::Function",
        {"Handler": "users_create.handler.handler"},
    )


def test_lambda_list_users_handler(template: Template) -> None:
    template.has_resource_properties(
        "AWS::Lambda::Function",
        {"Handler": "users_list.handler.handler"},
    )
