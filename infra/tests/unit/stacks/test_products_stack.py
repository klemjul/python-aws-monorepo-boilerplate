"""CDK assertion tests for ProductsStack."""
...
def _make_template(stack_id: str = "TestStack") -> Template:
    """Synthesise ProductsStack and return its CloudFormation template."""
    pairs = [_make_mock_pair() for _ in range(5)]
    ...
    with (
        ...
        app = cdk.App()
        stack = ProductsStack(app, stack_id)
...
def test_lambda_create_product_handler(template: Template) -> None:
    template.has_resource_properties(
        "AWS::Lambda::Function",
        {"Handler": "products_create.handler.handler"},
    )


def test_lambda_list_products_handler(template: Template) -> None:
    template.has_resource_properties(
        "AWS::Lambda::Function",
        {"Handler": "products_list.handler.handler"},
    )
