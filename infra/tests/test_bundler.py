...
def test_try_bundle_skips_install_when_nothing_to_install(tmp_path: Path) -> None:
    """try_bundle returns True and skips uv pip install when export output
    contains no installable requirements (only comments/annotations).
    """
    bundler = DepsBundler("/fake/source")
    export_mock = MagicMock()
    export_mock.returncode = 0
    # export produces only non-installable lines (comments/annotations)
    # — install must be skipped
    export_mock.stdout = "# autogenerated header\n    # via hello\n"
    export_mock.stderr = ""

    mock_run = MagicMock(return_value=export_mock)
...
# ---------------------------------------------------------------------------
# try_bundle - platform warning
# ---------------------------------------------------------------------------


def test_try_bundle_warns_on_non_linux(tmp_path: Path) -> None:
    """try_bundle emits a RuntimeWarning when running on a non-Linux platform."""
    bundler = DepsBundler("/fake/source")
    mock_run = _mock_subprocess_run(export_rc=0, install_rc=0)
    with (
...
def test_try_bundle_no_warning_on_linux(tmp_path: Path) -> None:
    """try_bundle must not emit any warning when running on Linux."""
    bundler = DepsBundler("/fake/source")
    mock_run = _mock_subprocess_run(export_rc=0, install_rc=0)
    with (
        patch.object(sys, "platform", "linux"),
